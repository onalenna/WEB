    ...
    <nav class="navbar navbar-inverse">
      <div class="container-fluid">
        ...
        <ul class="nav navbar-nav " >
          <li class="active"><a href="music.php">Home</a></li>
      ...
      <div class="rw">
        <div class="col-sm-2" style="background-image: url('img/bg/Advert.jpeg');">
          ...
          <div class="list-group" >
            <a href="music.php" class="list-group-item list-group-item-warning" >EXPLORE &nbsp;<span class="glyphicon glyphicon-search" style="color:transparent">.</span></a>
            ...
            <div class="col-sm-8">
              <form class="form-group"  method="POST">
                <div class="input-group">
                  <input type="text"  class="form-control" placeholder="Search music..." name="searchString" required="required" autocomplete="off">
...
//  echo " id is". $seachId;
                  $query=("SELECT * FROM music WHERE id='$seachId' LIMIT 1");
                  ...
                  while($all_video=mysqli_fetch_array($result))
                  {
                    ...
                    <object width="100%">
                      <div class="embed-responsive embed-responsive-16by9">
                        <iframe class="embed-responsive-item" src="audio/<?php echo $all_video['trackName']; ?>" autoplay="autoplay" type="audio/mp3" allowfullscreen ></iframe>
                      </div>
                    </object>
                    <div class="panel" style="background-color: #ffb460">
                      <i style="float: left;display: inline-block;"> 
                          ...
                          <?php //if($row=mysqli_fetch_assoc($res)){?>
                            <div class=" d-inline p-2"  >
                              <a href="javascript:void(0)" class="btn btn-info btn-xs">
                                <span class="glyphicon glyphicon-thumbs-up" onclick="like_update('<?php echo $all_video['id']?>')"> (<span id="like_loop_<?php echo $all_video['id']?>"><?php echo $all_video['likes']?></span>)</span>
                              </a> 
                              <a href="javascript:void(0)" class="btn btn-info btn-xs">
                                <span class="glyphicon glyphicon-thumbs-down" onclick="dislike_update('<?php echo $all_video['id']?>')"> (<span id="dislike_loop_<?php echo $all_video['id']?>"><?php echo $all_video['dislike']?></span>)</span>
                              </a>
                              <a href="javascript:void(0)" class="btn btn-info btn-xs">
                                <span class="glyphicon glyphicon-eye-open"> (<span id="like_loop_<?php echo $all_video['id']?>"><?php echo $all_video['views']?></span>)</span>
                              </a>
                            </div>
                            <?php// }?>
                          </form>
                        ...
                        <i style="float: right;">
                          <?php echo $all_video['trackName']." ~". $all_video['artist'];?> 
                        </i>
                      </div>
                      <?php
                      $list=$all_video['album'];
                      $selected=$all_video['id'];
                    }
                    ?>
                  </center>
                </div>
                ...
                <?php
                $query1=("SELECT * FROM music where album='$list' AND id!='$seachId'");
                ...
                while($all_video=mysqli_fetch_array($result1))
                {
                  ...
                  <div class="col-sm-4" style="background-color: transparent; overflow: auto;">
                    <img src="img/t6.jpg" style="width: 80%">
                    <?php echo "<h6>".$all_video['trackName']." ~ ".$all_video['artist']."</h6>";?>
                    <form method="post" >
                      <input type="hidden" name="id" value="<?php echo $all_video['id']?>">
                      <input type="submit" class="btn-success" value="select track"name="test" onclick="view_update('<?php echo $all_video['id']?>')">
                    </form>
                  </div>
                  <?php
                }
...
//   $name=$_SESSION['email'];
                ...
                $getThumb="SELECT * from music  where artist LIKE '%$searchString%' OR genre like '%$searchString%' OR trackName like '%$searchString%' OR album like '%$searchString%' OR description like '%$searchString%' or phonicTxt like '%$searchTxt%'";
...
// echo " ".$row['artist'];
                  if($row){
                    ...
                    <div class="col-sm-4">
                      <div class="well" style="border-color: #ffb460; overflow: auto">
                        <img src="img/t2.jpg" style="width: 80%;"  >
                        <?php echo "<h6>".$row['trackName']." ~ ".$row['artist']."</h6>";?>
                        <form method="post">
                          <input type="hidden" name="id" value="<?php echo $row['id']?>">
                          <input type="submit" class="btn-success" value="select track"name="test" onclick="view_update('<?php echo $row['id']?>')">
                        </form>
                      </div>
                    </div> 
                    <?php
                ...
                include("config.php");
//   $name=$_SESSION['email'];
                $getThumb="SELECT * from music  ORDER BY RAND() limit 8 offset 0";
                $rs = mysqli_query($conn,$getThumb) or die(mysqli_error($conn));
                while($row = mysqli_fetch_assoc($rs))
                {
                  ?>
                  <div class="col-sm-4" >
                    <div class="well" style="background-color: #ffb460;opacity: 100%;overflow: auto;">
                      ...
                      <img src="img/music.png" style="width: 80%;"  >
                      <?php echo "<h6>".$row['artist']." ~ ".$row['trackName']."</h6>"?>
                      <form method="post">
                        <input type="hidden" name="id" value="<?php echo $row['id']?>">
                        <center><br>
                          <input type="submit" class="btn-success" value="select track"name="test" onclick="view_update('<?php echo $row['id']?>')">
...
// echo "<br>random pics for you";
                  include("config.php");
//   $name=$_SESSION['email'];
                  $getThumb="SELECT * from music  ORDER BY RAND() limit 8";
                  $rs = mysqli_query($conn,$getThumb) or die(mysqli_error($conn));
                  while($row = mysqli_fetch_assoc($rs))
                  {
                    ?>
